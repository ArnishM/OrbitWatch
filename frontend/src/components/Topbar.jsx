...
const NOTIFICATIONS = [
  { id: 1, icon: '⚠',  text: 'Water level decreasing in Nagpur', time: '2h ago' },
  { id: 2, icon: '🌳', text: 'Tree cover improving in Nashik North', time: '5h ago' },
  { id: 3, icon: '🏙', text: 'Urban expansion accelerating — East Zone', time: '1d ago' },
  { id: 4, icon: '🔥', text: 'Heat island detected in Pune Cantonment', time: '1d ago' },
];
...
const Topbar = ({ district, setDistrict, year, setYear }) => {
  ...
  const handleSearch = (val) => {
    setSearch(val);
    setSuggestions(val.length > 0 ? DISTRICTS.filter(d => d.toLowerCase().includes(val.toLowerCase())).slice(0, 6) : []);
  };

  const selectDistrict = (d) => { setSearch(d); setSuggestions([]); if (setDistrict) setDistrict(d); };

  return (
    <header style={{
      height: 60, background: 'var(--topbar-bg)', backdropFilter: 'blur(20px)',
      borderBottom: '1px solid var(--border)', padding: '0 20px',
      display: 'flex', alignItems: 'center', justifyContent: 'space-between',
      flexShrink: 0, zIndex: 10, transition: 'background 0.3s ease'
    }}>
        ...
        <div style={{ position: 'relative' }}>
          ...
          <input value={search} onChange={e => handleSearch(e.target.value)}
            ...
            style={{
              ...
              borderRadius: 8, padding: '6px 28px 6px 32px', fontSize: 13,
        ...
        <div style={{ display: 'flex', gap: 3, background: 'rgba(128,128,128,0.08)', border: '1px solid var(--border)', borderRadius: 8, padding: 3 }}>
          {YEARS.map(y => (
            <button key={y} onClick={() => setYear && setYear(y)}
              style={{ padding: '4px 12px', borderRadius: 6, fontSize: 12.5, fontWeight: 500, fontFamily: 'IBM Plex Sans', cursor: 'pointer',
      ...
      {/* Right: status + theme toggle + notifications */}
      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
        {/* Live indicator */}
        ...
        <button id="theme-toggle" onClick={toggleTheme}
          style={{ width: 68, height: 32, borderRadius: 16, padding: '3px 4px', cursor: 'pointer', position: 'relative', transition: 'all 0.3s ease', background: theme === 'dark' ? 'rgba(33,150,243,0.15)' : 'rgba(251,191,36,0.15)', border: `1px solid ${theme === 'dark' ? 'rgba(33,150,243,0.3)' : 'rgba(251,191,36,0.4)'}`, display: 'flex', alignItems: 'center' }}>
          {/* Track */}
          ...
          {/* Thumb */}
          <motion.div animate={{ x: theme === 'dark' ? 36 : 0 }} transition={{ type: 'spring', stiffness: 400, damping: 30 }}
            ...
            {theme === 'dark' ? <FiMoon size={11} style={{ color: 'white', position: 'absolute', inset: 0, margin: 'auto', display: 'block' }} /> : <FiSun size={11} style={{ color: 'white', position: 'absolute', inset: 0, margin: 'auto', display: 'block' }} />}
          </motion.div>
        </button>
        ...
        <div ref={notifRef} style={{ position: 'relative' }}>
          <button id="notifications-btn" onClick={() => setShowNotif(!showNotif)}
            style={{ position: 'relative', background: 'rgba(128,128,128,0.08)', border: '1px solid var(--border)', borderRadius: 8, width: 34, height: 34, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', color: 'var(--text-secondary)' }}>
            <FiBell size={16} />
          ...
          <AnimatePresence>
            {showNotif && (
              <motion.div initial={{ opacity: 0, y: -8, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }}
                style={{ position: 'absolute', top: '115%', right: 0, width: 300, background: 'var(--bg-surface)', border: '1px solid var(--border)', borderRadius: 12, zIndex: 200, overflow: 'hidden', boxShadow: 'var(--shadow)' }}>
                <div style={{ padding: '12px 14px', borderBottom: '1px solid var(--border)', display: 'flex', justifyContent: 'space-between' }}>
                  <span style={{ fontWeight: 600, fontSize: 13, fontFamily: 'IBM Plex Serif, serif', color: 'var(--text-primary)' }}>Alerts</span>
